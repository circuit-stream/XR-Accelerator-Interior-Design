Shader "Custom/ARCorePaintableBackgroundShader"
{
    Properties
    {
...
#extension GL_OES_EGL_image_external_essl3 : require
            ...
            void Unity_ReplaceColor_float(vec3 In, vec3 From, vec3 To, float Range, out vec3 Out, float Fuzziness)
            {
                float Distance = distance(From, In);
                float clamped = clamp((Distance - Range) / max(Fuzziness, 1e-5f), 0.0, 1.0);
                Out = mix(To, In, clamped);
            }
